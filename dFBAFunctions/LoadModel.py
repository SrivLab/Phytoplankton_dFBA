...
def LoadModel(Inputs): 
    ...
    # Input photon bounds for 20 nm bins as in this model.
    for k in Inputs['Iphoto'].keys():
        ML.reactions.get_by_id(k).upper_bound = 0
        ML.reactions.get_by_id(k).lower_bound = Inputs['Iphoto'][k]*-1.
        ML.reactions.get_by_id(k).upper_bound = Inputs['Iphoto'][k]*-0.9999
